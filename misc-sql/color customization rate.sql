...
SELECT EXTRACT(YEAR
               FROM completed_at) order_year,
       EXTRACT(MONTH
               FROM completed_at) order_month,
       SUM(COALESCE(c.custom_color, 0) * li.price * li.quantity) / SUM(li.price * li.quantity) color_customization_rate
...
LEFT JOIN
  (SELECT lip.line_item_id,
   ...
   LEFT JOIN product_color_values pcv ON lip.color_id = pcv.id
   GROUP BY line_item_id) c ON c.line_item_id = li.id
  ...
  ( SELECT id
   ...
   WHERE NOT hidden
     AND (deleted_at IS NULL
          OR deleted_at > CURRENT_DATE)
     AND available_on <= CURRENT_DATE) p ON p.id = v.product_id
WHERE o.completed_at IS NOT NULL
  AND o.state = 'complete'
  AND o.payment_state = 'paid'
  AND o.shipment_state = 'shipped'
  AND o.completed_at >= '2015-01-01'
ORDER BY order_date,
         order_number -- products where all colors are custom

SELECT product_id,
       count(*) colors,
       sum(custom::int) custom_colors
FROM product_color_values
GROUP BY product_id HAVING count(*) = sum(custom::int) -- best method for calculating color customization rate must be done in R
-- sudo code calc
