                                    ...
                                    INITCAP(ss.name) ship_state,
                                                     INITCAP(sc.name) ship_country,
                                                                      o.completed_at::date order_date,
                                                                                           CASE
                                                                                               WHEN s.ship_date IS NULL THEN o.projected_delivery_date::DATE
                                                                                               ELSE s.ship_date::DATE
                                                                                           END ship_date,
                                                                                               o.email,
                                                                                               o.user_id,
                                                   ...
                                                   END return_order_id,
                                                       INITCAP(cust.color) color,
                                                                           ...
                                                                           UPPER(v.sku) style_number,
                                                                                        CASE
                                                                                            WHEN cust.height IS NULL THEN 'Not Specified'
                                                                                            ELSE INITCAP(cust.height)
                                                                                        END height,
                                                                                            RANK() OVER (PARTITION BY o.email
                                                                                                         ORDER BY o.completed_at) order_num,
                                                                                                                                  CASE
                                                                                                                                      WHEN tax.short_long IS NULL
                                                                                                                                           OR tax.short_long = 'Long,Short' THEN 'Not Specified'
                                                                                                                                      ELSE tax.short_long
                                                                                                                                  END short_long,
...
COALESCE(tax.bridal = 1, FALSE) bridal,
   ...
   LEFT JOIN spree_addresses sa ON sa.id = o.ship_address_id
...
( SELECT lip.line_item_id,
         MAX(CASE WHEN lip.customization_value_ids SIMILAR TO '%(1|2|3|4|5|6|7|8|9|0)%' THEN 1 ELSE 0 END) physical_customization,
...
STRING_AGG(DISTINCT lip.height, ',') height
 ...
 LEFT JOIN product_color_values pcv ON pcv.id = lip.color_id
...
(SELECT p.id product_id,
             STRING_AGG(DISTINCT CASE WHEN LOWER(t.name) SIMILAR TO '(mini|knee|petti)' THEN 'Short' WHEN LOWER(t.name) SIMILAR TO '(midi|ankle|maxi)' THEN 'Long' END, ',') short_long,
                                                                                                                                                                             MAX(CASE WHEN LOWER(t.name) LIKE '%skirt%'
                                                                                                                                                                                 OR LOWER(p.name) LIKE '%skirt%' THEN 1 ELSE 0 END) skirt,
