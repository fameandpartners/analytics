...
FROM spree_products_taxons pt
JOIN spree_taxons t ON t.id = pt.taxon_id
JOIN spree_products p ON p.id = pt.product_id -- All Categories
...
SELECT p.id product_id,
       ...
       MAX(CASE WHEN LOWER(t.name) = 'skirt'
...
WHERE t.name = 'Base Silhouette' --  product_id
-- ------------
--        1320
--        1317
--        1322
...
--        1316
 -- products with or without length
  ...
  SELECT sum(CASE WHEN lower(taxons) SIMILAR TO '%(maxi|midi|mini|knee|petti|ankle)%' THEN 1 ELSE 0 END) length_labeled,
         sum(1) no_length
  FROM
    ( SELECT p.id product_id,
             STRING_AGG(DISTINCT t.name, ' , ') taxons
     FROM spree_products_taxons pt
     JOIN spree_taxons t ON t.id = pt.taxon_id
     JOIN spree_products p ON p.id = pt.product_id
     GROUP BY p.id) t --  length_labeled | no_length
-- ----------------+-----------
--             497 |      1109