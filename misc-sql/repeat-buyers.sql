...
SELECT EXTRACT(YEAR
               FROM s.ship_date) ship_year,
       EXTRACT(MONTH
               FROM s.ship_date) ship_month,
       COUNT(*) orders,
...
FROM
  (SELECT *,
          RANK() OVER (PARTITION BY email
                       ORDER BY completed_at) which_order
   FROM spree_orders
                                                              ...
                                                              MIN(o.completed_at::date) OVER (PARTITION BY o.email) first_order_date,
                                                                                                                    MAX(o.completed_at::date) OVER (PARTITION BY o.email) last_order_date,
                                                                                                                                                                          RANK() OVER (PARTITION BY o.email
                                                                                                                                                                                       ORDER BY o.completed_at) which_order,
RANK() OVER (PARTITION BY o.email, p.id
             ORDER BY o.completed_at) same_product_order
...
SELECT o.created_at::DATE,
       s.ship_date,
       o.projected_delivery_date
FROM spree_orders o
LEFT JOIN
( SELECT order_id,
         MAX(shipped_at::DATE) ship_date
 FROM spree_shipments
 GROUP BY order_id) s ON s.order_id = o.id WHERE s.ship_date IS NULL