...
SELECT completed_at::DATE order_date,
       COUNT(*) items_sold,
...
INNER JOIN ( -- wedding app users

            SELECT DISTINCT user_id
            FROM wedding_atelier_users_event_roles) w ON w.user_id = o.user_id
...
GROUP BY order_date
ORDER BY order_date -- Orders from wedding app

SELECT COUNT(DISTINCT w.user_id) customers,
       COUNT(*) items_sold,
...
WHERE completed_at IS NOT NULL
  ...
  AND o.email NOT LIKE '%fameandpartners.com'
  AND o.email != 'gradient@gmail.com'
  AND o.total >= 0 -- boards

  SELECT owner_id board_owner,
         created_at::DATE board_create_date
  FROM wedding_atelier_events -- Boards and people on boards by day

  SELECT board.created_at::DATE board_ceated_date,
 ...
 -- total signups
  ...
  INNER JOIN
    ( SELECT id,
     ...
     FROM wedding_atelier_event_roles
     GROUP BY id) er ON er.id = uer.event_role_id
  SELECT er.first_wedding_event,
         u.created_at user_created_at,
         u.email
  FROM wedding_atelier_users_event_roles uer
  INNER JOIN
    ( SELECT id,
             MAX(created_at) first_wedding_event
     FROM wedding_atelier_event_roles
     GROUP BY id) er ON er.id = uer.event_role_id
  INNER JOIN spree_users u ON u.id = uer.user_id
ORDER BY er.first_wedding_event -- SELECT DISTINCT  er.signup_date, u.email FROM wedding_atelier_users_event_roles uer INNER JOIN ( SELECT id, MAX(created_at::DATE) signup_date FROM wedding_atelier_event_roles GROUP BY id) er ON er.id = uer.event_role_id INNER JOIN spree_users u  ON u.id = uer.user_id ORDER BY er.signup_date

SELECT inv.user_email,
       e.created_at::date
FROM wedding_atelier_invitations inv
JOIN wedding_atelier_events e ON inv.event_id = e.id
...
FROM wedding_atelier_invitations invite
INNER JOIN spree_users u ON u.email = invite.user_email
LEFT JOIN wedding_atelier_events board ON invite.event_id = board.id
LEFT JOIN wedding_atelier_users_event_roles uer ON uer.user_id = u.id
LEFT JOIN wedding_atelier_event_roles roles ON roles.id = uer.event_role_id
LEFT JOIN wedding_atelier_event_dresses dresses ON dresses.event_id = board.id
WHERE u.email NOT LIKE '%fameandpartners.com'
  AND u.email != 'gradient@gmail.com'
  AND roles.name IS NOT NULL
GROUP BY invite.state,
         roles.name
SELECT u.id IS NOT NULL fandp_user,
                        COUNT(DISTINCT invite.user_email),
                        COUNT(*)
FROM wedding_atelier_invitations invite
LEFT JOIN spree_users u ON u.email = invite.user_email
GROUP BY u.id IS NOT NULL -- users created during the 2/20 spike

SELECT EXTRACT(HOUR
               FROM first_wedding_event) signup_hour,
...
GROUP BY EXTRACT(HOUR
                 FROM first_wedding_event)
SELECT COUNT(DISTINCT users.user_id)
FROM wedding_atelier_events board
LEFT JOIN wedding_atelier_event_roles roles ON roles.resource_id = board.id
LEFT JOIN wedding_atelier_users_event_roles users ON users.event_role_id = roles.id
WHERE board.name IS NOT NULL
  SELECT created_at::DATE created_date,
         COUNT(*)
  FROM wedding_atelier_events board WHERE board.name IS NOT NULL
...
LEFT JOIN wedding_atelier_event_dresses d ON d.event_id = e.id
INNER JOIN spree_users u ON u.id = e.owner_id
GROUP BY e.created_at::DATE -- SELECT e.created_at::DATE signup_date,  COUNT(DISTINCT e.owner_id) brides, COUNT(DISTINCT d.id) dresses_added FROM wedding_atelier_events e LEFT JOIN wedding_atelier_event_dresses d ON d.event_id = e.id INNER JOIN spree_users u ON u.id = e.owner_id GROUP BY e.created_at::DATE