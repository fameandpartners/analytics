...
SELECT 
	...
	EXTRACT(EPOCH FROM MAX(m_order.created_at) - MIN(m_user.created_at))/60/60/24 days_to_sale
FROM spree_orders o
INNER JOIN marketing_order_traffic_parameters m_order
	ON o.id = m_order.order_id
LEFT JOIN marketing_user_visits m_user
	ON o.user_id = m_user.spree_user_id
...
SELECT utm1.order_id, utm1.utm_source, utm1.created_at::DATE
FROM marketing_order_traffic_parameters utm1
INNER JOIN marketing_order_traffic_parameters utm2 
	ON (utm1.order_id = utm2.order_id AND utm1.created_at < utm2.created_at)

FROM spree_orders o
INNER JOIN marketing_order_traffic_parameters utm
	ON o.id = utm.order_id
LEFT JOIN marketing_user_visits utm2
	ON o.user_id = utm2.spree_user_id
WHERE o.completed_at IS NOT NULL

-- monthly revenue last touch attribution by utm source
SELECT 
	EXTRACT(YEAR FROM motp.last_touch_date) last_touch_year,
	EXTRACT(MONTH FROM motp.last_touch_date) last_touch_month,
	motp.utm_source,
	SUM(o.total) revenue,
	COUNT(*) orders
FROM spree_orders o
...
INNER JOIN (
	...
	INNER JOIN (
		...
		WHERE utm_source SIMILAR TO '%([a-zA-Z])%'
		GROUP BY spree_user_id) muv2
		ON muv1.spree_user_id = muv2.spree_user_id AND muv1.created_at = muv2.first_touch_date
	GROUP BY muv1.spree_user_id) muv
	ON muv.spree_user_id = o.user_id
WHERE o.completed_at IS NOT NULL
	AND o.state = 'complete'
	AND o.payment_state = 'paid'
	AND o.total > 0
