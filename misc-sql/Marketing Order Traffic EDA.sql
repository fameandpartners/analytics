...
GROUP BY utm_medium
ORDER BY COUNT(DISTINCT utm_source || utm_medium || utm_campaign || order_id) DESC LIMIT 10;


SELECT o_utm.max_updated_at::DATE touch_date,
       o.state,
       o_utm.utm_source,
       COUNT(*) orders,
...
INNER JOIN spree_orders o ON o.id = o_utm.order_id
INNER JOIN (-- Limit to legit cart adds but keep at order level

            SELECT DISTINCT order_id
            FROM spree_line_items
            WHERE updated_at < CURRENT_DATE) li ON li.order_id = o.id
