...
SELECT 
	EXTRACT(YEAR FROM o.completed_at) completed_year,
	EXTRACT(MONTH FROM o.completed_at) completed_month,
	COUNT(DISTINCT o.id) completed_orders, 
	COUNT(DISTINCT CASE WHEN p.id IS NULL THEN o.id END) missing_payment
FROM spree_orders o
LEFT JOIN spree_payments p
...
GROUP BY 
	...
	EXTRACT(MONTH FROM o.completed_at);