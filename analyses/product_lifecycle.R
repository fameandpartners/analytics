...
library(gridExtra)
do.call("grid.arrange", c(monthly_dists, ncol=4))
...
product_rankings_per_quarter <- products_sold %>%
    ...
    mutate(quarterly_ranking = dense_rank(-net_return_request_units),
           performance_percentile = ntile(-net_return_request_units, 100) / 100,
    ...
    filter(between(performance_percentile, 0.05, 0.95))

quarterly_summaries <- product_rankings_per_quarter %>%
    group_by(order_year_quarter, performance_quintile) %>%
